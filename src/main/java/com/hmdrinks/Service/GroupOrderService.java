...
// Nếu now >= groupTime thì không cho phép
    ...
    public void processGroupOrder(GroupOrders groupOrder) {
        ...
        Map<Integer, List<CartItem>> cartItemsByVariant = cartItemRepository.findByProductVariants_VarIdIn((List<Integer>) variantIds)
                .stream().collect(Collectors.groupingBy(ci -> ci.getProductVariants().getVarId()));

        Map<Integer, List<CartItemGroup>> cartItemGroupsByVariant = cartItemGroupRepository.findByProductVariants_VarIdIn((List<Integer>) variantIds)
                .stream().collect(Collectors.groupingBy(cig -> cig.getProductVariants().getVarId()));
